...
import io.kontak.apps.anomaly.detector.detector.AverageAnomalyDetector;
...
public class AverageAnomalyDetectorTest {

    private final AnomalyDetector anomalyDetector = new AverageAnomalyDetector();
    ...
    public void testSimpleCaseAnomaly() {
        var readings = List.of(
                new TemperatureReading(21.0, "1", "1", Instant.parse("2023-07-30T12:00:00Z")),
                new TemperatureReading(29.3, "1", "1", Instant.parse("2023-07-30T12:00:00Z")),
                new TemperatureReading(20.5, "1", "1", Instant.parse("2023-07-30T12:00:00Z"))
        );
    ...
    public void testSimpleCaseAnomalies() {
        ...
        Assertions.assertEquals(29.3, anomaly.get(0).temperature());
        assertThat(anomaly, containsInAnyOrder(
                new Anomaly(31.8, "1", "1", Instant.parse("2023-07-30T12:10:00Z")),
                new Anomaly(29.3, "1", "1", Instant.parse("2023-07-30T12:02:00Z"))
        ));
    }
}
